...
pbsdcp $INP/'*' $TMPDIR
pbsdcp -r $INP/Output_GenConfig $TMPDIR
...
do 
	gmx_gpu grompp -f waterUmbrella.mdp -c Output_GenConfig/config_${i}ps.gro -n water.ndx -p water.top -o ${i}ps.tpr
	gmx_gpu mdrun -v -deffnm ${i}ps -nb gpu
	# retrieve output
	pbsdcp -g $TMPDIR/${i}ps'*' $OUT
done
